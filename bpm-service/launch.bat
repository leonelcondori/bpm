...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "bpm-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name bpm-service apps/bpm-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app bpm-service:1.0-SNAPSHOT
		call oc expose svc/bpm-service
	) else (
